...
public class SwimmingDino extends Actor {
	...
	/*
	 * Constructor for the swimming dinosaur starts the character in the middle
	 * row The character can only move up or down between rows
	 */
	...
	/*
	 * Divides the rectangle container into 3 rows. This allows the user to move
	 * between these 3 rows upon swiping. The speed of switching a row is 0.35
	 */
	private void moveToLane(int paramInt) {
		...
		case 0:
			float f3 = getX();
			this.game.getClass();
			addAction(Actions.moveTo(f3, this.game.row1 - getHeight() / 2.0F,
					0.35F));
			return;
		case 1:
			float f2 = getX();
			this.game.getClass();
			addAction(Actions.moveTo(f2, this.game.row2 - getHeight() / 2.0F,
					0.35F));
			return;
		case 2:
			float f1 = getX();
			this.game.getClass();
			addAction(Actions.moveTo(f1, this.game.row3 - getHeight() / 2.0F,
					0.35F));
			return;
		default:
			return;
	...
	public void draw(SpriteBatch paramSpriteBatch, float paramFloat) {
		...
		paramSpriteBatch.draw(Assets.playerDino, getX(), getY(),
				getWidth() / 2.0F, getHeight() / 2.0F, getWidth(), getHeight(),
				1.0F, 1.0F, getRotation());
	} // End draw
