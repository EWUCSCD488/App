...
public GameOverScreen(GameSetup setup, PlantesFerry game) {
	...
	this.menuTable.add(this.gameoverLabel).expandX().center().padBottom(this.buttonPaddingBottom1).row();
	this.menuTable.add(this.scoreLabel).expandX().center().padBottom(this.buttonPaddingBottom2).row();
	this.menuTable.add(this.highscoreLabel).expandX().center().padBottom(this.buttonPaddingBottom1).row();
	this.menuTable.add(this.restartGameButton).expandX().center().padBottom(this.buttonPaddingBottom2).row();
...
public void resize(int width,int height) {
    this.stage.setViewport(this.game.screenWidth, this.game.screenHeight, true);
    this.stage.getCamera().translate(-this.stage.getGutterWidth(), -this.stage.getGutterHeight(), 0.0F);
} // End resize

/*
 * Renders the Game Over Screen
 * Displays the end game score, high score and asks user to try again or return to map
 */
@Override
public void render(float delta) {
	Gdx.gl.glClearColor(0.0F, 0.0F, 0.0F, 1.0F);
		...
		this.paramSpriteBatch.draw(Assets.menubg, 0.0F, 0.0f, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
